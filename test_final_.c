...
int cost_function_(int ts_l2_M, int ts_l2_N, int ts_l2_K,
                   int ts_l1_M, int ts_l1_N, int ts_l1_K,
                   int unused1, int unused2, int unused3, int unused4)
{
  clock_t start = clock();
...
int test_final()
{
  ...
  generate_search_space(parameters, 6, &search_space);
  ...
  for (int i = 0; i < search_space_size; ++i)
  {
        ...
        ts_l1_M_index < 0 || ts_l1_N_index < 0 || ts_l1_K_index < 0)
    {
      ...
      if (ts_l2_M_index == -1)
        printf("Es fehlt Parameter ts_l2_M.\n");
      if (ts_l2_N_index == -1)
        printf("Es fehlt Parameter ts_l2_N.\n");
      if (ts_l2_K_index == -1)
        printf("Es fehlt Parameter ts_l2_K.\n");
      if (ts_l1_M_index == -1)
        printf("Es fehlt Parameter ts_l1_M.\n");
      if (ts_l1_N_index == -1)
        printf("Es fehlt Parameter ts_l1_N.\n");
      if (ts_l1_K_index == -1)
        printf("Es fehlt Parameter ts_l1_K.\n");
      exit(EXIT_FAILURE);
    }
  ...
  if (not_found_configs_exist)
    exit(EXIT_FAILURE);

  // start exploration
  configuration_t best_config;
  int best_cost;
  explore_search_space(&search_space, cost_function_,
